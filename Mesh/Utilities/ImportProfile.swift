...
typealias PrefillResponse = (first_name: String, last_name: String, email: String, image_url: String)
...
struct LinkedInProfile {
    static func prefill(_ completion: @escaping ((PrefillResponse) -> Void)) {
        let linkedinHelper = LinkedinSwiftHelper(configuration: LinkedinSwiftConfiguration(clientId: "775eh4hg8ks2le", clientSecret: "HHTipcKAKIH1DtfK", state: "DLKDJF45DIWOERCM", permissions: ["r_basicprofile", "r_emailaddress"], redirectUrl: "http://google.com"))
...
//                let position = ((response.jsonObject["positions"] as? JSONDictionary)?["values"] as? JSONArray)?.first
                completion(PrefillResponse(firstName, lastName, email, imageURL))
            })
            }, error: { (error) -> Void in
                print(error)
            }, cancel: { () -> Void in
